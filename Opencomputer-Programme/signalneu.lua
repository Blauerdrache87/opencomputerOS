...
function install()
  ...
  print("Requirements:")
  print("Tier 2 screen")
  print("Tier 2 graphics card")
  print("A BigReactor connected by cable from computer port to computer")
  print("A keyboard for install only, optional for practical use")
  print()
  print("Are all requirements met? (y/n)")
  --21,49
  local result = false
  while not result do
...
function draw_menubar()
  ...
  gpu.setForeground(config.color_scheme.menubar.foreground)
  term.write(" Fuel Temperature: ")
  gpu.setForeground(config.color_scheme.info)
  term.write(round(reactor.getFuelTemperature()).."C ")
  gpu.setForeground(config.color_scheme.menubar.foreground)
  term.write(" Casing Temperature: ")
  gpu.setForeground(config.color_scheme.info)
...
function draw_main()
  if config.auto_power.enabled then
    ...
    gpu.fill(1,2,69,3," ")
    term.setCursor(25,3)
    term.write("Disable Auto-Power")
    gpu.setBackground(0x153F3F)
    gpu.fill(70,2,11,3," ")
    term.setCursor(71,3)
    term.write("Configure")
  else
    gpu.setBackground(config.color_scheme.button)
    gpu.setForeground(0xFFFFFF)
    gpu.fill(1,2,69,3," ")
